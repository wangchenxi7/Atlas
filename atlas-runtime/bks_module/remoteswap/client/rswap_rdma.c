...
int rswap_create_rdma_queue(struct rdma_session_context *rdma_session, int rdma_queue_index)
{
	...
	if (rdma_session->rdma_dev == NULL) {
		...
		pr_info("selecting device %s\n", rdma_session->rdma_dev->pd->device->name);
	...
	cq_num_cqes = max(rdma_session->send_queue_depth, rdma_session->recv_queue_depth);
...
int init_rdma_sessions(struct rdma_session_context *rdma_session)
{
	...
	rdma_session->send_queue_depth = RDMA_SEND_QUEUE_DEPTH;
	rdma_session->recv_queue_depth = RDMA_RECV_QUEUE_DEPTH;
...
int rswap_init_rdma_queue(struct rdma_session_context *rdma_session, int idx)
{
	...
	rdma_queue->fs_rdma_early_map_req_cache = kmem_cache_create(
        "fs_rdma_early_map_req_cache", sizeof(struct fs_rdma_early_map_req), 0,
        SLAB_TEMPORARY | SLAB_HWCACHE_ALIGN, NULL);
    if (unlikely(rdma_queue->fs_rdma_early_map_req_cache == NULL)) {
        printk(KERN_ERR
               "%s, allocate rdma_queue->fs_rdma_early_map_req_cache failed.\n",
               __func__);
	...
	rdma_queue->fs_rdma_obj_req_cache = kmem_cache_create(
        "fs_rdma_obj_req_cache", sizeof(struct fs_rdma_obj_req), 0,
        SLAB_TEMPORARY | SLAB_HWCACHE_ALIGN, NULL);
    if (unlikely(rdma_queue->fs_rdma_obj_req_cache == NULL)) {
        printk(KERN_ERR "%s, allocate rdma_queue->fs_rdma_obj_req_cache failed.\n",
               __func__);
        kmem_cache_destroy(rdma_queue->fs_rdma_req_cache);
        kmem_cache_destroy(rdma_queue->fs_rdma_early_map_req_cache);
        ret = -1;
        goto err;
    }
