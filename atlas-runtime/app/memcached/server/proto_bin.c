...
#include <stdlib.h>
#include "atlas.h"
static bool atlas_enable = false;
...
static void complete_update_bin(conn *c) {
    ...
    ret = store_item(it, c->cmd, c, &it_up);
...
static void process_bin_get_or_touch(conn *c, char *extbuf) {
    ...
    if (strncmp(key, "00000006", 8) == 0) {
        atlas_disable_runtime_fetch();
        fprintf(stderr, "Disable runtime fetch\n");
        printf("Disable runtime fetch\n");
    }

    if (strncmp(key, "00000007", 8) == 0) {
        atlas_enable_runtime_fetch();
        fprintf(stderr, "Enable runtime fetch\n");
        printf("Enable runtime fetch\n");
    }
