...
void resp_reset(mc_resp *resp) {
    if (resp->item) {
        printf("resp reset start ...\n");
        atlas_item_remove(resp->item, resp->item_up);
        printf("resp reset end ...\n");
        resp->item = NULL;
        resp->item_up = NULL;
    }
...
mc_resp* resp_finish(conn *c, mc_resp *resp) {
    ...
    if (resp->item) {
        ...
        // printf("resp finish start ...\n");
        atlas_item_remove(resp->item, resp->item_up);
        // printf("resp finish end ...\n");
        resp->item = NULL;
        resp->item_up = NULL;
    }
    if (resp->write_and_free) {
...
enum store_item_type do_store_item(item *it, int comm, conn *c, const uint32_t hv, atlas_unique_ptr **it_up) {
    ...
    if (old_it != NULL) {
        ...
        /* [Atlas] after item_replace, the old_up will be destroyed, so we dec the refcnt here */
        ...
        if (do_store) {
            stored = do_item_link(it, hv, it_up);
        }
    }
