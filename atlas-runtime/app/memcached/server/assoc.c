...
void assoc_init(const int hashtable_init) {
    ...
    mlock(primary_hashtable, hashsize(hashpower)*sizeof(void *));
    printf("hashtable locked, size: %lu MB\n", sizeof(void *) * hashsize(hashpower) >> 20);
    STATS_LOCK();
...
int atlas_assoc_insert_get(item *it, const uint32_t hv, atlas_unique_ptr**up) {
    BARRIER_ASSERT(!expanding);
    bool need_evac = is_atlas_ptr(primary_hashtable[hv & hashmask(hashpower)]);
    if (need_evac) {
        return 0;
    }
    ...
    if(need_evac){
        atlas_up_mark_evacuation((atlas_unique_ptr*)(&primary_hashtable[hv & hashmask(hashpower)]));
    }
    ...
    *up = (atlas_unique_ptr*)&primary_hashtable[hv & hashmask(hashpower)];
    if(need_evac){
        atlas_up_clear_evacuation((atlas_unique_ptr*)&it->h_next);
    }
...
void assoc_delete(const char *key, const size_t nkey, const uint32_t hv) {
    ...
    if (it) {
        ...
        bool need_evac = is_atlas_ptr(it->h_next);
        if(need_evac){
            atlas_up_mark_evacuation((atlas_unique_ptr*)&it->h_next);
        }
        nxt = it->h_next;
        ...
        *before = nxt;
        if(need_evac){
            atlas_up_clear_evacuation((atlas_unique_ptr*)before);
        }
        return;
    }
