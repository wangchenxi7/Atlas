...
static const char *adc_counter_names[NUM_ADC_COUNTER_TYPE] = {
	...
	"HermitReclaim  ", "Optimisic Faild", "EarlyPopulatePages", 
	"EarlyMapPtes", "AsyncPrefPages", "MinorMapFailed", "PrefetchNotMatch",
	"DirectMapFailed", "LazyUpdateMetadata",
};
...
static const char *adc_time_stat_names[NUM_ADC_TIME_STAT_TYPE] = {
	...
	"async prefetch overhead",
	"RMAP1_IPC          ", "RMAP2_IPC          ", "TLB_FLUSH_DIRTY_IPC",
	"TLB_FLUSH_IPC      ", "RDMA_WRITE_IPC     ", "HREMIT RMAP1_IPC   ",
	"HERMIT RMAP2_IPC   ", 
};
	...
	for (type = 0; type < NUM_ADC_TIME_STAT_TYPE; type++) {
		...
		if (type < NUM_ADC_LAT_TYPE) { // latencies
			...
			printk("%s: %lldns, #: %lld\n",
			       ...
			       (int64_t)atomic_read(&ts->cnt));
		} else if (__is_defined(ADC_PROFILE_IPC)) { // IPCs
			printk("%s: %lld/1000, #: %lld\n",
			       adc_time_stat_names[type], dur,
			       (int64_t)atomic_read(&ts->cnt));
		}
	}
