...
void __mmdrop(struct mm_struct *mm)
{
	...
	// [Hermit] drop global dsa list
	hermit_cleanup_mm(mm);
	// [Hermit] end
	mm_free_pgd(mm);
	destroy_context(mm);
					...
					struct kernel_clone_args *args)
{
	...
	/* [Hermit] */
	p->is_hermit_cthd = args->is_hermit_cthd;
	// init the thread private dsa
	init_dsa(&p->dsa);
	// DSA list
	spin_lock_init(&p->dsl_lock);
	INIT_LIST_HEAD(&p->dsa_list);
	atomic_set(&p->dsa_cnt, 0);
	p->hermit_sthd = NULL;
	/* [Hermit] end */
...
pid_t kernel_clone(struct kernel_clone_args *args)
{
	...
	put_pid(pid);
	/* [Hermit] */
	if (args->is_hermit_cthd) {
		int ret;
		pr_err("%s:%d create sthd for %s=>%s", __FUNCTION__, __LINE__,
		       current->comm, p->comm);
		// dump_stack();
		ret = hermit_sthd_run(p);
	}
	return nr;
}
